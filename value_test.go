...
func TestValueCAD(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		value, err := test.Money.Value()
		if nil != err {
			t.Errorf("For test #%d, did not expect an error but receive one: %v", testNumber, err)
		}

		s, ok := value.(string)
		if !ok {
			t.Errorf("For test #%d, expected returned value to actually be a string, but wasn't.", testNumber)
		}
